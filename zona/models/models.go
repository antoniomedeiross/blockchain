...
// VotoConsensus é enviado como resposta a um BLOCO_PROPOSTA.
// Aceito=true significa que o peer validou o bloco e concorda em adicioná-lo.
type VotoConsensus struct {
	HashBloco string `json:"hash_bloco"` // identifica qual proposta está sendo votada
	Aceito    bool   `json:"aceito"`
	Motivo    string `json:"motivo,omitempty"` // preenchido quando Aceito=false
}