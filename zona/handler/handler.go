...
func ProcessarConexoes(conn net.Conn) {
	...
	for {
		...
		case "BLOCO_PROPOSTA":
			// Outro peer minerou um bloco e está pedindo nossa aprovação (consenso 2/3).
			// Validamos o bloco e respondemos com VOTO_CONSENSO.
			dadosJSON, _ := json.Marshal(mensagem.Dados)
			var bloco ledger.Bloco
			if err := json.Unmarshal(dadosJSON, &bloco); err != nil {
				log.Printf("[CONSENSUS] ✗ Erro ao parsear BLOCO_PROPOSTA: %v\n", err)
				continue
			}
			...
			if !bloco.Validar() {
				...
				// Valida saldo se for pagamento
				if aceito && bloco.Tx.Tipo == ledger.TxPagamento {
					saldo := ledger.ConsultarSaldo(bloco.Tx.ZonaID)
					if saldo < bloco.Tx.Creditos {
						aceito = false
			...
			if aceito {
				log.Printf("[CONSENSUS] ✔ Proposta #%d de %s aprovada — hash %s\n", bloco.Index, peerZona, bloco.Hash[:12])
			} else {
				log.Printf("[CONSENSUS] ✗ Proposta #%d de %s rejeitada: %s\n", bloco.Index, peerZona, motivo)
			}

			// Responde com o voto
			voto := models.VotoConsensus{
				HashBloco: bloco.Hash,
				Aceito:    aceito,
				Motivo:    motivo,
			}
			resposta := models.Mensagem{
		...
		case "VOTO_CONSENSO":
			// Recebi o voto de um peer sobre uma proposta que fiz.
			// Encaminha para o canal correto em PendingConsensus (blockchain package).
			dadosJSON, _ := json.Marshal(mensagem.Dados)
			var voto models.VotoConsensus
			if err := json.Unmarshal(dadosJSON, &voto); err != nil {
				log.Printf("[CONSENSUS] ✗ Erro ao parsear VOTO_CONSENSO: %v\n", err)
				continue
			}
			if ch, ok := blockchain.PendingConsensus.Load(voto.HashBloco); ok {
				ch.(chan bool) <- voto.Aceito
			} else {
				log.Printf("[CONSENSUS] ⚠ Voto recebido mas proposta %s não encontrada (já expirou?)\n", voto.HashBloco[:12])
			}
		...
		case "REQUISICAO_DRONE":
			...
			// Descarta requisições inválidas (zona ou sensor vazios) para não travar a fila
			if req.ZonaID == "" || req.Sensor == "" {
				log.Printf("[FILA] ⚠ Requisição inválida descartada (zona='%s' sensor='%s')\n", req.ZonaID, req.Sensor)
				continue
			}
		...
		case "SYNC_REQUEST":
			...
			// Registra a chave pública do peer para validar assinaturas futuras
			if pk, ok := payload["pubkey"].(string); ok && pk != "" {
				ledger.RegistrarChavePeer(peerZona, pk)
				log.Printf("[IDENTIDADE] ✔ Chave pública de %s registrada via SYNC\n", peerZona)
			}
